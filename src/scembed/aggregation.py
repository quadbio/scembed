...
class scIBAggregator:
    ...
    def fetch_runs(
            ...
            filter_allow_none: bool = True) -> None:
        ...
        logger.info(
            "Processing complete: %d methods, %d runs filtered out due to missing config/metrics", len(self.method_data), self.n_runs_filtered
        )

    def _filter_params(self, target_key, target_value) -> None:
        """Filter runs to match certain parameter criteria. It allows to filter by any key available in the config."""
